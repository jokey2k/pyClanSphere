...
"""
    ...
    For display strings are translated to the language of the site and all
    dates as converted to the preferred timezone.
    ...
    pyClanSphere loads afterwards are stored in pickles rather than MO files.
    ...
    The extracting of strings (either for pyClanSphere core or plugins) is done
...
import pyClanSphere
from pyClanSphere.environment import LOCALE_PATH, LOCALE_DOMAIN, \
     USE_GETTEXT_LOOKUP
...
def load_core_translations(locale):
    ...
    This only loads the translations for pyClanSphere itself and not the
    ...
    """
    return pyClanSphereTranslations.load(LOCALE_PATH, locale, LOCALE_DOMAIN,
                                 USE_GETTEXT_LOOKUP)
...
class pyClanSphereTranslations(TranslationsBase, _CustomAttrsTranslations):
...
class pyClanSphereNullTranslations(NullTranslations, _CustomAttrsTranslations):
...
def get_translations():
    ...
    try:
        return pyClanSphere.application.get_application().translations
...
def per_language_string(en, **languages):
    ...
    """
    def lookup():
        app = pyClanSphere.application.get_application()
...
def to_clan_timezone(datetime):
    """Convert a datetime object to the preferred timezone."""
...
def get_timezone(name=None):
    ...
    """
    if name is None:
        name = pyClanSphere.application.get_application().cfg['timezone']
...
def serve_javascript(request):
    ...
    if code is None:
        ...
        code = 'pyClanSphere.addTranslations(%s)' % dump_json(dict(
            messages=dict((k.id, k.string) for k in t.client_keys),
            plural_expr=t.plural_expr,
            locale=str(t.locale)
        ))
        _js_translations[request.app] = code
    response = pyClanSphere.application.Response(code, mimetype='application/javascript')
    response.add_etag()
