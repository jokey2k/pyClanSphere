...
"""
    ...
    To identify a plugin metadata are used. pyClanSphere requires a file
    named `metadata.txt` to load some information about the plugin.

    pyClanSphere currently supports the following metadata information:
...
from pyClanSphere.utils.mail import split_email, is_valid_email, check
from pyClanSphere.utils.exceptions import UserException, summarize_exception
from pyClanSphere.i18n import pyClanSphereTranslations as Translations, lazy_gettext, _
from pyClanSphere.environment import BUILTIN_PLUGIN_FOLDER
...
def get_object_name(obj):
    ...
    if inspect.isclass(obj) or inspect.isfunction(obj):
        ...
        cls = obj.__class__
    if cls.__module__.startswith('pyClanSphere.plugins.'):
        prefix = cls.__module__.split('.', 2)[-1]
    elif cls.__module__.startswith('pyClanSphere.'):
...
def install_package(app, package):
    ...
    try:
        package_version = int(f.read('PYCLANSPHERE_PACKAGE'))
        plugin_name = f.read('PYCLANSPHERE_PLUGIN')
...
class InstallationError(UserException):
    ...
    MESSAGES = {
        ...
        'version':  lazy_gettext('The plugin uploaded has a newer package '
                                 'version than this pyClanSphere installation '
                                 'can handle.'),
...
class Plugin(object):
    ...
    def dump(self, fp):
        ...
        for name, data in [('PYCLANSPHERE_PLUGIN', self.name),
                           ('PYCLANSPHERE_PACKAGE', PACKAGE_VERSION)]:
    ...
    def module(self):
        ...
        try:
            # we directly import from the pyClanSphere module space
            return __import__('pyClanSphere.plugins.%s' % self.name, None, None,
                              ['setup'])
        except:
...
sys.modules['pyClanSphere.plugins'] = _plugins = ModuleType('pyClanSphere.plugins')
