...
try:
    ...
    from pyClanSphere._core import MissingDependency
...
from pyClanSphere import __version__ as VERSION, setup
# imported for side-effects
from pyClanSphere.upgrades import customisation
...
class CommandLineInterface(object):
    ...
    def run(self, argv=sys.argv):
        ...
        self.parser.add_option('-I', '--instance', dest='instance',
                               help="pyClanSphere instance path")
    ...
    def cmdlogger(self, messages):
        from pyClanSphere.utils.zeml import parse_html  # late import
    ...
    def list(self, argv):
        self.get_pyClanSphere_instance()
        def list_them():
    ...
    def script(self, argv):
        ...
        parser.add_option('-r', '--repository-id', default='pyClanSphere', dest='repo_id',
                          help='the repository id (default: pyClanSphere)')
        parser.add_option(
            '-f', '--filename',
            help="file name (without spaces and/or version number)")
        options, args = parser.parse_args(argv)
        description = ' '.join(args)
        manage = ManageDatabase(self.get_pyClanSphere_instance())
...
class ManageDatabase(object):
    ...
    def cmd_upgrade(self, repo_id=None, version=None, **opts):
        ...
        from pyClanSphere.models import SchemaVersion
        if repo_id is None:
            # get all repos, sort those with id "pyClanSphere*" first
            available_svs = sorted(
                SchemaVersion.query.order_by(SchemaVersion.repository_id.asc()),
                key=lambda sv: not sv.repository_id.startswith('pyClanSphere'))
        else:
