...
"""
    ...
    would already be returned by the normal pyClanSphere installation.
...
from pyClanSphere.config import ConfigurationTransactionError
from pyClanSphere.models import User
from pyClanSphere.utils.crypto import gen_pwhash, gen_secret_key, new_iid
from pyClanSphere.utils.validators import is_valid_email, check
from pyClanSphere.i18n import load_core_translations, has_language, list_languages
...
CONFIG_HEADER = '''\
# pyClanSphere configuration file
# This file is also updated by the pyClanSphere admin interface.
...
class WebSetup(object):
    """Minimal WSGI application for installing pyClanSphere"""
    ...
    def start_setup(self, request):
        ...
        try:
            ...
            config_filename = path.join(self.instance_folder, 'pyClanSphere.ini')
            ...
            cfg._comments['[pyClanSphere]'] = CONFIG_HEADER
            try:
                t.commit()
